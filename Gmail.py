...
def get_user(user_id, name="User", username=""):
    ...
    if uid not in data["users"]:
        data["users"][uid] = {
            ...
            "referred_by": None,
            "ref_rewarded": False,
            "referral_list": [],  # list of dicts: {"id", "name", "username"}
...
def get_main_menu(user_id):
    ...
    if lang == "en":
        markup.add(KeyboardButton("💰 𝑩𝒂𝒍𝒂𝒏𝒄𝒆"), KeyboardButton("👥 𝑹𝒆𝒇𝒆𝒓𝒓𝒂𝒍"))
...
def get_admin_menu():
    markup = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    markup.add(KeyboardButton("📢 Set Channel"), KeyboardButton("🎁 Set Ref Bonus"))
    markup.add(KeyboardButton("💳 Set Min Withdraw"), KeyboardButton("🔘 Manage Button"))
    markup.add(KeyboardButton("🛠️ Manage Method"), KeyboardButton("📥 Pending New Mail"))
    markup.add(KeyboardButton("📥 Pending Old Mail"), KeyboardButton("📥 Pending Use mail"))
    markup.add(KeyboardButton("💸 Pending Withdraw"), KeyboardButton("📢 Broadcast"))
    markup.add(KeyboardButton("⛔ Ban User"), KeyboardButton("🟢 Unban User"))
    markup.add(KeyboardButton("📊 Bot Statistics"), KeyboardButton("🔑 New Mail Pass Set"))
    markup.add(KeyboardButton("➕ Add Mainmenu Button"), KeyboardButton("➕ Add Balance"))
    markup.add(KeyboardButton("➖ Cut balance"), KeyboardButton("📜 All User Details"))
    markup.add(KeyboardButton("🏷️ Set Mail Prices"), KeyboardButton("🔴 Maintenance Mode"))
    markup.add(KeyboardButton("📁 Export Data"), KeyboardButton("🔎 User Search & History"))
...
def start_cmd(message):
    ...
    # ভাষা সিলেকশন না থাকলে ভাষা সিলেক্ট করতে বলা
...
def callback_handler(call):
    ...
    if call.data == "verify_join":
        ...
        if left:
            ...
            try:
                bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=get_force_join_markup(left))
            except:
                pass
            return
        ...
        # ২-৩ মিনিটের টাইম লজিক (মাঝামাঝি ১২০ সেকেন্ড ধরা হয়েছে)
        if elapsed < 120:
            ...
            markup = InlineKeyboardMarkup(row_width=2)
            markup.add(
                InlineKeyboardButton("✅ Approve", callback_data=f"appr_new_{req_key}"),
                InlineKeyboardButton("❌ Reject", callback_data=f"rej_new_{req_key}")
            )
        ...
        if target_dict and req_key in target_dict:
            ...
            if action == "appr":
                ...
                # ৩টি মেইল সেল হলে রেফার অ্যাক্টিভ করা
...
def handle_text_messages(message):
    ...
    if state == "enter_old_mail_address":
        if "@gmail.com" not in text:
            bot.send_message(message.chat.id, "❌ সঠিক জিমেইল এড্রেস লিখুন:")
            return
        ...
        markup = InlineKeyboardMarkup(row_width=2)
        markup.add(
            InlineKeyboardButton("✅ Approve", callback_data=f"appr_{m_type}_{req_key}"),
            InlineKeyboardButton("❌ Reject", callback_data=f"rej_{m_type}_{req_key}")
        )
        ...
        try:
            ...
            bot.send_message(message.chat.id, "🕊️ <b>আপনার উইথড্র রিকোয়েস্ট সফলভাবে জমা হয়েছে। ধৈর্য ধরে অপেক্ষা করুন, খুব দ্রুত পেমেন্ট কমপ্লিট করা হবে!</b>", parse_mode="HTML", reply_markup=get_main_menu(user_id))

            # এডমিন মেসেজ বিস্তারিত
            admin_msg = (f"💸 <b>NEW WITHDRAWAL REQUEST!</b>\n\n"
                         ...
                         f"💰 Amount: ৳<b>{amt:.2f}</b>\n"
                         f"📊 Total Sales: {user['success_mails']}\n"
                         f"👥 Total Ref: {len(user['referral_list'])}\n"
                         f"⏰ Time: {time.strftime('%Y-%m-%d %H:%M:%S')}")
            
            markup = InlineKeyboardMarkup(row_width=2)
            markup.add(
                InlineKeyboardButton("✅ Approve", callback_data=f"wappr_{w_key}"),
                InlineKeyboardButton("❌ Reject", callback_data=f"wrej_{w_key}")
            )
    ...
    if int(user_id) == ADMIN_ID and state:
        if state == "admin_set_channel":
            ...
            save_db(data)
            bot.send_message(message.chat.id, f"✅ মেইন মেনুতে নতুন বাটন যুক্ত হয়েছে: {text}")
            update_user(user_id, "state", None)
            return
    ...
    if text in ["💰 মোট ব্যালেন্স", "💰 𝑩𝒂𝒍𝒂𝒏𝒄𝒆"]:
        ...
        markup.add(InlineKeyboardButton("📋 আপনার রেফারেল লিস্ট দেখুন", callback_data="show_ref_list"))
        ...
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton("✅ Subject / জিমেইল খোলা শেষ", callback_data="submit_new_mail_check"))
        bot.send_message(message.chat.id, msg, parse_mode="HTML", reply_markup=markup)
        ...
        for m, enabled in data.get("withdraw_methods", {}).items():
            if enabled: btns.append(InlineKeyboardButton(m, callback_data=f"with_select_{m}"))
        markup.add(*btns)
        bot.send_message(message.chat.id, msg, parse_mode="HTML", reply_markup=markup)
    ...
    elif text == "➖ Cut balance" and int(user_id) == ADMIN_ID:
        update_user(user_id, "state", "admin_cut_bal")
        bot.send_message(message.chat.id, "লিখুন: `USER_ID AMOUNT`", parse_mode="Markdown")

    elif text == "⛔ Ban User" and int(user_id) == ADMIN_ID:
        update_user(user_id, "state", "admin_ban_user")
        bot.send_message(message.chat.id, "Ban করার জন্য USER_ID লিখুন:")

    elif text == "🟢 Unban User" and int(user_id) == ADMIN_ID:
        update_user(user_id, "state", "admin_unban_user")
        bot.send_message(message.chat.id, "Unban করার জন্য USER_ID লিখুন:")

    elif text == "🏷️ Set Mail Prices" and int(user_id) == ADMIN_ID:
        update_user(user_id, "state", "admin_set_prices")
        bot.send_message(message.chat.id, "লিখুন: `NEW_PRICE OLD_PRICE USED_PRICE`", parse_mode="Markdown")

    elif text == "🔎 User Search & History" and int(user_id) == ADMIN_ID:
    ...
    elif text == "🔴 Maintenance Mode" and int(user_id) == ADMIN_ID:
    ...
    elif text == "📁 Export Data" and int(user_id) == ADMIN_ID:
...
if __name__ == "__main__":
    keep_alive()  # Render Web Server Activator
